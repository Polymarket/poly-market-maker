...
class GasStation:
    ...
    def __init__(self, strat=GasStrategy.STATION, w3: web3.Web3 = None, url: str = None, fixed=DEFAULT_FIXED_GAS_PRICE):
    ...
    def _get_rpc_gas_price(self):
        try:
            ...
            self.logger.error(f"Error fetching gas from web3, returning configured fixed gas price: {self.fixed}")
            return self.fixed
    ...
    def _get_gas_station_gas(self):
        try:
            ...
            self.logger.error(f"Error fetching gas from gasstation, returning configured fixed gas price: {self.fixed}")
            return self.fixed
